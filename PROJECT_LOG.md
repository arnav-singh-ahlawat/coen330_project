...
| 2026-06-13 | Niraj | Ran comprehensive pre-report reproducibility review from project root. | Verify the full COEN 330 project before report/PDF work and document exactly what happened. | `results/full_reproducibility_run.txt`, `docs/comprehensive_project_review.md`, `PROJECT_LOG.md`, `report/REPORT_NOTES.md`, regenerated tracked result CSVs | Passed: `python -m src.preprocessing`, `python -m src.eda`, `python -m src.train`, `python -m src.evaluate`, `python -m src.stratified_baseline`, and `python demo/demo.py`. Confirmed required outputs, plots, model artifact, processed local data, and demo. | No ML methodology changed. The event-aware event-4 result remains a failure to generalize: precision, recall, F1, and F2 are 0.0 with 330 false negatives and 0 true positives. Remaining issues: final PDF missing, team contribution placeholders need real names/contributions, and final submission package is not prepared. |
| 2026-06-13 | Niraj | Created final submission report materials and checklist. | Prepare the final COEN 330 submission materials from the verified draft, result tables, plots, audit, and project notes. | `report/final_report.md`, `report/final_report.pdf`, `docs/final_submission_checklist.md`, `PROJECT_LOG.md`, `report/REPORT_NOTES.md`, `docs/comprehensive_project_review.md` | Created final report Markdown; generated final report PDF locally using a temporary HTML rendering plus LibreOffice headless conversion; verified the PDF with `pdfinfo` and `pdfimages -list`; created the final submission checklist. | Documentation-only change. No training/evaluation code, labels, failure windows, event-aware split logic, model selection logic, thresholds, reported metrics, or generated result CSV values were changed. Team contribution names remain `TODO_NAME` placeholders, and the final Moodle ZIP still needs to be prepared. |
| 2026-06-13 | Niraj | Added review-oriented Jupyter notebooks and documented them in submission materials. | Make the repository look like a complete ML project while keeping `src/` as the authoritative reproducible pipeline. | `notebooks/01_project_overview.ipynb`, `notebooks/02_preprocessing_and_eda.ipynb`, `notebooks/03_model_training_and_validation.ipynb`, `notebooks/04_evaluation_and_error_analysis.ipynb`, `notebooks/05_demo_usage.ipynb`, `README.md`, `docs/final_submission_checklist.md`, `report/final_report.md`, `report/final_report_draft.md`, `report/REPORT_NOTES.md`, `PROJECT_LOG.md` | Added explanation-heavy notebooks that load existing results and plots; updated README, checklist, report files, report notes, and project log. | Documentation-only change. No preprocessing logic, training logic, evaluation logic, labels, failure windows, event-aware split, model selection, thresholds, generated metrics, or generated plots were changed. |
